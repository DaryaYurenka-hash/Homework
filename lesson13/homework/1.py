...
class User:
	...
	def login(self, value):
		...
		# С применением re
		if not re.fullmatch(r'[A-Za-z0-9_]{6,}', value): #6 раз и более если необходимо проверяем соответствуют ли все символы
...
#  re.fullmatch(r'[A-Za-z0-9_]{6,}', 'User_12') >>> <Match> /
# re.fullmatch(r'[A-Za-z0-9_]{6,}', 'User-12') >>> None

# re.compile — компилирует регулярное выражение для многократного использования.
# pattern = re.compile(r'\d+')
# pattern.search('abc123') >>> <Match>

# re.escape - экранирует все специальные символы, превращая строку в безопасный шаблон. re.escape('a.b*c?') >>> 'a\\.b\\*c\\?'

# re.findall — возвращает список всех совпадений в строке.
# re.findall(r'\d+', 'ab12cd34') >>> ['12', '34']

# re.finditer - возвращает итератор с объектами совпадений (с позициями).
# list(re.finditer(r'\d+', 'ab12cd34')) >>> [<Match>, <Match>]

# re.match - ищет совпадение только в начале строки.
# re.match(r'\d+', '123abc') >>> <Match>
# re.match(r'\d+', 'abc123') >>> None

# re.purge — очищает кэш скомпилированных регулярных выражений.
# re.purge() >>> None
...
# re.sub - заменяет все совпадения на указанную строку.
# re.sub(r'\d+', '*', 'ab12cd34') >>> 'ab*cd*'

# re.subn - делает замену и возвращает кортеж: (результат, количество замен).
# re.subn(r'\d+', '*', 'ab12cd34') >>> ('ab*cd*', 2)
